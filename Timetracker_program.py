...
def createCSV():
    ...
    # creates file.csv with columns only
        ...
        columns = pd.DataFrame(
            columns=['Type', 'Current Date', 'Task name', 'Started', 'Ended', 'hours', 'Amount earned $'])
        # save file.csv
...
if choice == 1:
    ...
    while (
    True):  # accepting input for the start time in the format specified in the print statement.It also checks for a valid time
        ...
        if k[2] == ":" and k[-2:] in ["AM", "PM"] and (int(k[:2]) >= 0 and int(k[:2]) <= 12) and (
                int(k[3:5]) >= 0 and int(k[3:5]) <= 59):
            break  # if valid then break

            print("Retry!")

    while (
    True):  # accepting input for the end time in the format specified in the print statement.It also checks for a valid time
...
# Entering the LIVE Project mode
    ...
    print(f"Good!!!you have spent {hour} hours...")  # prints the number of hours worked
    print(f"Awesome! you have made ${round(total_money, 2)}...")  # prints amount of money made